...
import PlayPause from './PlayPause'; // Ensure the path is correct based on your project structure
...
const SongCard = ({
  ...
  <div className="flex flex-col w-[250px] p-4 bg-white/5 bg-opacity-80 backdrop-blur-sm rounded-lg cursor-pointer">
    <div className="relative w-full h-56 group">
      ...
      >
        <PlayPause
          isPlaying={isPlaying && activeSong?.title === song.title}
    ...
    <div className="mt-4 flex flex-col">
      ...
      <p className="text-sm truncate text-gray-300 mt-1">
        <Link to={`/artists/${song.subtitle}`}>{song.subtitle}</Link>
      </p>
    </div>
  </div>
);
